...
/** socketId -> { id, pseudo, avatarInitials, avatarColor, phone, phoneKey, doorOpen, roomId, contacts: Set<phoneKey> } */
const users = new Map();

/** roomId -> { hostId, memberIds: Set<socketId> } */
const rooms = new Map();

/** Normalise un numéro pour la comparaison (garde uniquement les chiffres). */
...
// (comparaison sur le numéro normalisé), pas tous les comptes connectés.
...
io.on('connection', (socket) => {
  ...
  socket.on('register', ({ pseudo, avatarInitials, avatarColor, phone }) => {
    const user = {
      ...
      doorOpen: false,
      roomId: null,
      contacts: new Set(), // phoneKey des contacts ajoutés par CE compte
    };
  ...
  // 1bis) Ajout d'un contact par numéro de téléphone.
  ...
  // 4) Un ami clique sur "Rejoindre" -> demande envoyée à l'hôte, qui doit valider.
  socket.on('call:request', ({ hostId }) => {
    ...
    io.to(hostId).emit('call:incoming-request', publicUser(me));
  });
  ...
  // L'hôte accepte : on prévient le demandeur, qui va récupérer son micro
  // puis confirmer via 'call:ready' avant de rejoindre réellement la room.
