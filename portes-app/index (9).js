...
io.on('connection', (socket) => {
  ...
  socket.on('register', ({ pseudo, avatarInitials, avatarColor, phone, contacts }) => {
    const user = {
      id: socket.id,
...
.avatar-preview{
  ...
  display:flex; align-items:center; justify-content:center;
  font-family:'Baloo 2', sans-serif; font-weight:700; font-size:24px; color:#fff;
  box-shadow:0 8px 18px -10px rgba(0,0,0,0.5);
}
.avatar-preview-hint{ font-size:12px; font-weight:700; color:rgba(0,0,0,0.55); }
.emoji-grid{
  display:grid; grid-template-columns:repeat(8, 1fr); gap:4px;
  background:rgba(255,255,255,0.6); border-radius:12px; padding:6px; margin-bottom:8px;
}
.emoji-choice{
  aspect-ratio:1; border:none; background:transparent; border-radius:9px; cursor:pointer;
  font-size:17px; line-height:1; display:flex; align-items:center; justify-content:center;
}
.emoji-choice.selected{ background:var(--ink); }
.color-row{ display:flex; gap:6px; margin-bottom:4px; }
.color-choice{
  flex:1; height:24px; border-radius:8px; border:2px solid transparent; cursor:pointer;
}
  ...
  <div class="screen login-screen" id="loginScreen">
    <div class="login-inner">
      ...
      <div class="avatar-preview-row">
        ...
        <div class="avatar-preview-hint">Choisis une tête et une couleur 👇<br>(laisse vide pour garder tes initiales)</div>
      </div>
      <div class="emoji-grid" id="avatarEmojiGrid"></div>
      <div class="color-row" id="avatarColorRow"></div>
...
function buildAvatarPicker() {
  ...
  AVATAR_EMOJIS.forEach((emo) => {
    ...
    b.addEventListener('click', () => {
      pickedEmoji = (pickedEmoji === emo) ? '' : emo;
      refreshAvatarPicker();
    });
    grid.appendChild(b);
  });
  ...
  \$('pseudoInput').addEventListener('input', () => {
    if (!colorPickedByHand) pickedColor = colorForPseudo(\$('pseudoInput').value.trim() || 'a');
    refreshAvatarPicker();
  });
...
function refreshAvatarPicker() {
  ...
  Array.from(grid.children).forEach((b, i) => {
    b.classList.toggle('selected', pickedEmoji !== '' && AVATAR_EMOJIS[i] === pickedEmoji);
  });
  Array.from(\$('avatarColorRow').children).forEach((b, i) => {
    b.classList.toggle('selected', palette[i] === pickedColor);
  });
  const pseudo = \$('pseudoInput').value.trim();
  paintAvatar(\$('avatarPreview'), pickedEmoji || (pseudo ? initialsFor(pseudo) : '--'), pickedColor);
}
...
socket.on('registered', (user) => {
  ...
  paintAvatar(\$('headerAvatar'), user.avatarInitials, user.avatarColor);
  paintAvatar(\$('myAvatar'), user.avatarInitials, user.avatarColor);
...
function boot() {
  ...
  if (profile && profile.pinHash) {
    ...
    paintAvatar(\$('lockAvatar'), avatarTextFor(profile), profile.avatarColor);
